...
<body>
    <div class="panel admin-panel">
        ...
        <div class="padding border-bottom">
            <ul>
                <li>
                    <a  class="button border-main icon-plus-square-o" href="item/add">添加订单</a>


                    <input type="text" placeholder="请输入订单号" name="code" value="${itemOrder.code}" class="input" style="width: 250px;line-height: 17px;display: inline-block">
        ...
        <table class="table table-hover text-center">
            ...
            <c:forEach items="${pagers.datas}" var="data" varStatus="1">
                <tr>
                    ...
                    <td style="color: red">
                    <c:if test="${data.status == 0}">未发货</c:if>
                    <c:if test="${data.status == 1}">待收货</c:if>
                    <c:if test="${data.status == 2}">已收货</c:if>
